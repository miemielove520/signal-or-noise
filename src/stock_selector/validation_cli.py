"""The ``validate`` command: walk-forward validation of one universe, or of all built-in ones.

Shared by ``validate.py`` and ``stock-selector validate``.
"""
...
def run_validation(args: argparse.Namespace, fail: Callable[[str], NoReturn]) -> int:
    """Run validation for already-parsed (and preset-applied) ``args``.
